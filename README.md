...
- [Validación y reflexión](#validación-y-reflexión)
...
**Cómo se probó.** El juego se ejecutó y se probó en **Visual Studio Code**, con **Python** y la librería **Pygame** instalados. Cada iteración se corría directamente (`python neon_drift.py`) para validar la jugabilidad y la sensación de control, y durante el desarrollo también se usaron pruebas automáticas sin interfaz (Pygame en modo headless) para comprobar la lógica antes de revisar el resultado visual.

**Un error / limitación y su ajuste.** Hubo varios; estos dos son los más representativos:

- **Limitación de diseño — "simulador de caminar".** La primera versión tenía una sola órbita, así que cualquier punto que apareciera en tu anillo era gratis: no había decisión ni reto, solo esperar. **Ajuste:** se rediseñó la mecánica de raíz a tres órbitas con física de Kepler y fotones que orbitan a la velocidad de su anillo, lo que obliga a un *encuentro orbital* (cambiar de anillo, dejar que el ángulo se cierre y cruzar en el momento justo). Eso convirtió la recolección en un puzzle de tiempo y le dio un propósito a cada segundo.

- **Error técnico — recogida que se "escapaba".** Las colisiones se comprobaban frame a frame, de modo que a velocidades relativas altas el jugador podía atravesar un fotón sin recogerlo; y caer a ~10 px de uno y aun así no atraparlo se sentía injusto (en el mismo anillo no puedes cerrar ese hueco residual porque ambos van a la misma velocidad). **Ajuste:** una **zona de atracción invisible** (~48 px) que atrae el fotón hacia el jugador en los últimos píxeles hasta el contacto, sin inflar el radio visible de recogida ni romper el rendezvous. Perdona el "casi" sin que el jugador tenga que pensarlo.

### Reflexión final

**Qué aprendí usando IA para programar.** Que hay una facilidad increíble: se puede programar un videojuego desde cero y sin assets sin tener que tocar casi nada de código a mano.

**Ventajas y límites del *vibecoding*.**

*Ventajas:* la velocidad para producir código es enorme, y eso permite mantener el foco en las **mecánicas y lo visual** en lugar de en cómo implementarlas. Cosas que normalmente exigirían leer documentación, ver tutoriales y administrar tiempo para aprender, practicar e implementar —algo aparentemente simple como las "órbitas" o la física de movimiento de los meteoritos— se resuelven sin frenar el diseño.

*Límites:* la IA genera el código que le pidas, pero **no fabrica los assets**, y **no llega a la idea final a la primera** ni con el mejor prompt del mundo, porque muchas veces hace falta la **interacción humana** para probar cómo va quedando y decidir qué ajustar. A veces incluso se necesita el **input de alguien externo** al proyecto para ver cosas que tú ya no ves desde dentro: para la IA "es obvio" lo que está pasando y no lo explica del todo bien a menos que se lo pidas, gastando tokens en algo que quizá vendría mejor de una persona ajena. Y la IA **no sustituye el diseño visual enteramente creativo**: tendería a copiar algo que ya existe o muy parecido a aquello con lo que fue entrenada. La creatividad nace de las ideas y la imaginación; la IA puede ayudarte a materializarlas con base en lo que ya sabe, pero le cuesta expresar tu idea **tal cual es** si nunca vio algo similar.

**Qué comprendo y qué necesito reforzar.** Por la complejidad de lo que se construyó, admito que entiendo poco del código en sí. Necesito un **refuerzo completo de Python y sus librerías** para entender a fondo todo lo que abarca un mini-juego como este.
